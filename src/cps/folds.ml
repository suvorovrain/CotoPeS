...
let fk x s k = (fun t -> k (s ^ t)) (string_of_int x)
let s x y = x + y
let sk x y k = k (x + y)
let list = [ 1; 2; 3; 4 ]
...
let%expect_test "fold left list" =
  (try print_string (fold_left f "" list) with
   | Stack_overflow -> print_endline "Stack overflow!");
  [%expect {| 1234 |}]
;;

let%expect_test "fold right list" =
  (try print_string (fold_right f list "") with
   | Stack_overflow -> print_endline "Stack overflow!");
  [%expect {| 4321 |}]
;;

let%expect_test "fold left cps1 list" =
  (try print_string (fold_left_cps1 f "" list Fun.id) with
   | Stack_overflow -> print_endline "Stack overflow!");
  [%expect {| 1234 |}]
;;

let%expect_test "fold right cps1 list" =
  (try print_string (fold_right_cps1 f list "") with
   | Stack_overflow -> print_endline "Stack overflow!");
  [%expect {| 4321 |}]
;;

let%expect_test "fold left cps2 list" =
  (try print_string (fold_left_cps2 fk "" list) with
   | Stack_overflow -> print_endline "Stack overflow!");
  [%expect {| 1234 |}]
;;

let%expect_test "fold right cps2 list" =
  (try print_string (fold_right_cps2 fk list "" Fun.id) with
   | Stack_overflow -> print_endline "Stack overflow!");
  [%expect {| 4321 |}]
;;

let%expect_test "fold right cps2 (with helper) list" =
  (try print_string (fold_right_cps2_wh fk list "") with
   | Stack_overflow -> print_endline "Stack overflow!");
  [%expect {| 4321 |}]
;;
...
let%expect_test "fold left list" =
  (try print_int (fold_left s 0 huge_list) with
   | Stack_overflow -> print_endline "Stack overflow!");
  [%expect {| 10000000 |}]
;;

(* fold left хорош без мам пап и cps'ов*)

let%expect_test "fold right list" =
  (try print_int (fold_right s huge_list 0) with
   | Stack_overflow -> print_endline "Stack overflow!");
  [%expect {| Stack overflow! |}]
;;

let%expect_test "fold left cps1 list" =
  (try print_int (fold_left_cps1 s 0 huge_list Fun.id) with
   | Stack_overflow -> print_endline "Stack overflow!");
  [%expect {| 10000000 |}]
;;

let%expect_test "fold right cps1 list" =
  (try print_int (fold_right_cps1 s huge_list 0) with
   | Stack_overflow -> print_endline "Stack overflow!");
  [%expect {| 10000000 |}]
;;

let%expect_test "fold left cps2 list" =
  (try print_int (fold_left_cps2 sk 0 huge_list) with
   | Stack_overflow -> print_endline "Stack overflow!");
  [%expect {| 10000000 |}]
;;

let%expect_test "fold right cps2 list" =
  (try print_int (fold_right_cps2 sk huge_list 0 Fun.id) with
   | Stack_overflow -> print_endline "Stack overflow!");
  [%expect {| 10000000 |}]
;;
